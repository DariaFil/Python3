...
class CLifegame(object):
    def __init__(self, _height, _length):
        self.height = _height
        self.length = _length
        self.game_field = [[None]*self.length for i in range(self.height)]
    ...
    def next_game_state(self):
        next_state_game_field = [[None]*self.length for l in range(self.height)]
        for i in range(self.height):
            for j in range(self.length):
                obj = self.dictSymbol_to_object[self.game_field[i][j]]
